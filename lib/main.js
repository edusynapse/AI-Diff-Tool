...
// -------------------------
// History export: prompt Save dialog and write a ZIP chosen by user
// Renderer sends base64 payload (no new deps).
// -------------------------
ipcMain.handle('history:saveZip', async (_evt, payload) => {
  try {
    const filenameRaw = String(payload?.filename || 'AI-Diff-History.zip').trim() || 'AI-Diff-History.zip';
    const filename = path.basename(filenameRaw).endsWith('.zip') ? path.basename(filenameRaw) : `${path.basename(filenameRaw)}.zip`;
    const b64 = String(payload?.data_b64 || '');
    if (!b64) return { ok: false, reason: 'no_data' };
    ...
    let outPath = String(res.filePath);
    if (!outPath.toLowerCase().endsWith('.zip')) outPath += '.zip';
...
// -------------------------
// Save "Download Modified File" output with a Save dialog
// - If payload.defaultDir exists, open the dialog there
// - Otherwise fall back to Downloads
// -------------------------
ipcMain.handle('output:saveText', async (_evt, payload) => {
  try {
    const filenameRaw = String(payload?.filename || 'file.txt').trim() || 'file.txt';
    const filename = path.basename(filenameRaw); // prevent path traversal
    const text = (payload?.text == null) ? '' : String(payload.text);

    let defaultDir = String(payload?.defaultDir || '').trim();
    try {
    ...
    await fsp.writeFile(String(res.filePath), text, 'utf8');
    return { ok: true, path: String(res.filePath) };
  } catch {
    return { ok: false, reason: 'write_failed' };
  }
});
...
// -------------------------
// Right-click context menu (Diff + File Content textareas only)
// Renderer requests a minimal native menu with exactly:
// Select All / Copy / Paste / Delete
// -------------------------
ipcMain.on('rightClickMenu:show', (evt, payload) => {
  const win = BrowserWindow.fromWebContents(evt.sender);
  if (!win || win.isDestroyed()) return;

  const x = Math.round(Number(payload?.x) || 0);
  const y = Math.round(Number(payload?.y) || 0);
  const can = (payload && typeof payload === 'object' && payload.can && typeof payload.can === 'object')
    ? payload.can
    : {};
  ...
  const lbl = (k, fb) => {
    const v = String(labels?.[k] || '').trim();
    return v || String(fb || '');
  };

  const menu = Menu.buildFromTemplate([
    { role: 'selectAll', label: lbl('selectAll', 'Select All'), enabled: !!can.selectAll },
    { role: 'copy',      label: lbl('copy',      'Copy'),      enabled: !!can.copy },
    { role: 'paste',     label: lbl('paste',     'Paste'),     enabled: !!can.paste },
    { role: 'delete',    label: lbl('delete',    'Delete'),    enabled: !!can.delete }
  ]);

  try { menu.popup({ window: win, x, y }); } catch {}
});
