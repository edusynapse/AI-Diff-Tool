...
function closeAllModalsForLanguageChange() {
  ...
  try { initHistoryManagerOnce().closeHistoryModal(); } catch {}
  ...
  try { closeCancelApiModal({ force: true }); } catch {}
...
const CONFIRM_APPLY_MAXTOKENS_LS_PREFIX = 'confirmApply.maxTokens.'; // +providerId
const CONFIRM_APPLY_MAXTOKENS_DEFAULT_OPENAI = 32768;
const CONFIRM_APPLY_MAXTOKENS_DEFAULT_XAI = 8192;
// New: fallback to model's manifest max_tokens if available (Grok models have huge values)
function _manifestMaxTokensForModel(modelId) {
  try {
    const meta = modelManifestIndex?.modelMeta?.get?.(modelId);
    if (meta && Number.isFinite(meta.max_tokens)) {
      return Math.floor(meta.max_tokens);
    }
  } catch {}
  return null;
}
...
function _providerForModelId(modelId) {
  ...
  // Prefer manifest index if present
  try {
    const p = modelManifestIndex?.modelToProvider?.get?.(m);
    if (p) return String(p);
  } catch {}

  // Fall back to apikeys manager mapping (single source of truth for runtime routing)
  try {
    const api = initApiKeysManagerOnce();
    const p = api?.providerForModel?.(m);
    if (p) return String(p);
  } catch {}
...
function _applyConfirmApplyTokensUi({ providerId, tabId } = {}) {
  ...
  if (def <= 0) {
    ...
    const manifestMax = _manifestMaxTokensForModel(modelId);
    if (manifestMax > 0) {
      def = manifestMax;
    } else {
...
function applyI18nToConfirmApplyModal(modelName) {
  ...
  const providerId = _providerForModelId(model);
...
function openConfirmApplyModal({ modelName, onOk, tabId } = {}) {
  ...
  overlay.dataset.providerId = _providerForModelId(overlay.dataset.modelName);
...
function wireConfirmApplyModalOnce() {
  ...
  if (okBtn) okBtn.addEventListener('click', () => {
    ...
    const providerId = String(overlayNow?.dataset?.providerId || '').trim()
      || _providerForModelId(overlayNow?.dataset?.modelName);
 ...
 function closeCancelApiModal({ force = false } = {}) {
   ...
   if (!overlay) return;
   overlay.classList.add('hidden');
   try { delete overlay.dataset.tabId; delete overlay.dataset.token; } catch {}
   _syncBodyModalOpen();
 }
 ...
 function wireCancelApiModalOnce() {
   ...
   if (overlay) {
     overlay.addEventListener('click', (e) => {
       if (e.target === overlay) closeCancelApiModal();
     });
   }
   ...
   document.addEventListener('keydown', (e) => {
     if (e.key !== 'Escape') return;
     if (_isOverlayOpen('cancelApiOverlay')) {
       e.preventDefault();
       e.stopPropagation();
       closeCancelApiModal();
     }
...
// -------------------------
// Model manifest (root/model_manifest.json)
// - Single source of truth for model dropdown.
// - Contains approximate pricing for future cost estimation (not used yet).
// -------------------------
...
function _findModelManifestPath() {
  const candidates = [
    path.resolve(process.cwd(), 'model_manifest.json'),
    path.resolve(__dirname, 'model_manifest.json'),
    path.resolve(__dirname, '..', 'model_manifest.json'),
    path.resolve(__dirname, '..', '..', 'model_manifest.json')
  ];
...
function _loadModelManifestSync() {
  ...
  // minimal validation + build indexes
  ...
  // require at least one model total
...
function buildModelSelectFromManifest(manifest) {
  ...
  // Clear any existing hardcoded options; manifest is now the source of truth.
  sel.replaceChildren();
  ...
  for (const prov of manifest.providers) {
    ...
    og.dataset.provider = providerId; // REQUIRED for existing gating logic
    ...
    for (const m of models) {
      ...
      // Keep pricing around for later UI/cost calc work (no behavior change today).
      try {
    ...
    // Only attach non-empty groups (prevents blank headers)
...
function initModelProviderGateOnce() {
 ...
 function updateSystemPromptButtonForTab(tab) {
   ...
   if (!btn) return;
   try { ensureIconButtonStructure(btn, ICON_SVG_PROMPT, 'Prompt'); } catch {}
...
window.addEventListener('DOMContentLoaded', async () => {
  ...
  // Build model dropdown from root/model_manifest.json (single source of truth)
  try {
    modelManifest = _loadModelManifestSync();
    if (modelManifest) buildModelSelectFromManifest(modelManifest);
  } catch (e) {
    console.error('Failed to load model_manifest.json:', e);
  }

  const fallbackDefaultModel =
    String(modelManifest?.defaults?.model || '').trim() ||
    'grok-4-fast-reasoning';

  // Restore selected model (and coerce to currently-available dropdown options)
  const storedModelRaw = localStorage.getItem('selectedModel') || fallbackDefaultModel;
  const storedModel = coerceModelToEnabled(storedModelRaw);
  const modelSel = document.getElementById('modelSelect');
  if (modelSel) modelSel.value = storedModel;
