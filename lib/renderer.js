...
function syncFilePickerNamesFromTab(tab) {
  // Display: prefer FULL PATH; fallback to just name
  const diffDisp = String(tab?.diffInputFilePath || tab?.diffInputFileName || '').trim();
  const modelDisp = String(
    tab?.modelInputFilePath ||
    tab?.originalFilePath ||
    tab?.modelInputFileName ||
    tab?.originalFileName ||
    ''
  ).trim();
  setFilePickerName('diff', diffDisp);
  setFilePickerName('model', modelDisp);
  ...
  try {
    const modelBase = String(tab?.modelInputFileName || tab?.originalFileName || '').trim();
    if (modelBase) originalFileName = modelBase;
  } catch { }
}

function _pickedFileInfoFromInput(inputEl) {
  try {
    const f = inputEl?.files?.[0];
    if (!f) return { name: '', path: '' };
    // Electron provides `File.path` for <input type="file"> selections
...
function initPerTabFilePickerState() {
  ...
  if (diffInput && diffInput.dataset.perTabWired !== '1') {
    ...
    diffInput.addEventListener('change', () => {
      ...
      const info = _pickedFileInfoFromInput(diffInput);
      if (tab) {
        tab.diffInputFileName = info.name;      // basename (keep for logic)
        tab.diffInputFilePath = info.path;      // full path (for display)
      }
      // Display full path if available
      setFilePickerName('diff', info.path || info.name);
    });
  }
  ...
  if (modelInput && modelInput.dataset.perTabWired !== '1') {
    ...
    modelInput.addEventListener('change', () => {
      const tab = (typeof getActiveTab === 'function') ? getActiveTab() : null;
      const info = _pickedFileInfoFromInput(modelInput);
      if (tab) {
        tab.modelInputFileName = info.name;     // basename
        tab.modelInputFilePath = info.path;     // full path
        if (info.name) tab.originalFileName = info.name;
        if (info.path) tab.originalFilePath = info.path;
      }
      if (info.name) originalFileName = info.name;
      setFilePickerName('model', info.path || info.name);
    });
  }
}
...
// - Open a Save dialog
// - Default directory = directory of the loaded "File Content" file (if exists)
// -------------------------
function _dirIfExists(p) {
  try {
    ...
    return st.isDirectory() ? dir : '';
  } catch {
    return '';
  }
}

function _defaultSaveDirForActiveTab() {
  const tab = (typeof getActiveTab === 'function') ? getActiveTab() : null;
  // Prefer full path of File Content input
  const p = tab?.originalFilePath || tab?.modelInputFilePath || '';
  return _dirIfExists(p);
}

async function _saveOutputWithDialog() {
  const tab = (typeof getActiveTab === 'function') ? getActiveTab() : null;

  // Grab output from DOM (safe even if tab storage differs)
  const outEl = document.getElementById('output');
  const text = String(outEl?.textContent || '');
  if (!text.trim()) return;

  const baseName = path.basename(String(tab?.originalFileName || originalFileName || 'file.txt'));
  const defaultDir = _defaultSaveDirForActiveTab();

  const res = await ipcRenderer.invoke('output:saveText', {
  ...
  if (res?.ok && tab) {
    try {
      tab.lastOutputSavePath = String(res.path || '');
      tab.lastOutputSaveDir = _dirIfExists(String(res.path || ''));
    } catch { }
  }
}

let _downloadBtnWired = false;
function wireDownloadButtonOnce() {
  if (_downloadBtnWired) return;
  const btn = document.getElementById('download');
  if (!btn) return;
  _downloadBtnWired = true;

  // Capture-phase so we override any existing download logic reliably
  btn.addEventListener('click', async (e) => {
    e.preventDefault();
    e.stopImmediatePropagation();
    try { await _saveOutputWithDialog(); } catch { }
  }, true);
}

if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', wireDownloadButtonOnce, { once: true });
} else {
  wireDownloadButtonOnce();
}
